...
char  lora_TXBUFF[128];			//buffer where transmitted packets are stored
...
byte lora_RXBUFFPrint(byte lora_LPrint)
{
  ...
  if (lora_LPrint == 3)
  {
    if ((lora_RXPacketType > 0x1F) &&  (lora_RXPacketType < 0x80))     //test for ASCII printable
    {
      Serial.write(lora_RXPacketType);                   //if its ASCII print it
    }
    else
    {
      Serial.print(F(" "));              //if control character print space
    }

    if ((lora_RXDestination > 0x1F) &&  (lora_RXDestination < 0x80))     //test for ASCII printable
    {
      Serial.write(lora_RXDestination);                   //if its ASCII print it
    }
    else
    {
      Serial.print(F(" "));              //if control character print space
    ...
    if ((lora_RXSource > 0x1F) &&  (lora_RXSource < 0x80))     //test for ASCII printable
    {
      Serial.write(lora_RXSource);                   //if its ASCII print it
    }
    else
    {
      Serial.print(F(" "));              //if control character print space
  ...
  for (byte i = lora_RXStart; i <= lora_RXEnd; i++)
  {
    ...
    {
      j = lora_RXBUFF[i];
      if ((j > 0x1F) &&  (j < 0x80))     //test for ASCII printable
      {
        Serial.write(j);                   //if its ASCII print it
      }
      else
      {
        Serial.print(F(" "));              //if control character print space
